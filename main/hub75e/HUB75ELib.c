...
void HUB75E_displayBufferPixels()
{
	...
	{
		for (int column = 0; column < HORIZONTAL_PIXELS_IN_BYTES; column++)
		{ // Shifting pixels for each row. HORIZONTAL_PIXELS_IN_BYTES x 8
			...
			for (int i = 7; i >= 0;)
			{ // Shifting 8 pixels horizontally. MSb First
		...
		switch (brightness)
		{ // More the delay, more the brightness
